...
def main():
    ...
    domains = list(train_data[0]['goal'].keys())
    for del_k in g.ignore_keys_in_goal:
        domains.remove(del_k)
    domains = list(sorted(domains))
    print('{} domains: {}'.format(len(domains), domains))

    # print some dialogs with Dialog States
    for d in train_data[:30]:
        print('-' * 50)
